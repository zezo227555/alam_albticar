                        ...
                        <tr>
                            ...
                            @if (auth()->user()->employee_salary_create)
                                <td>
                                    ...
                                    @if ($is_there)
                                        <form action="{{ route('employee.salary_update') }}" method="POST"
                                            class="d-inline-block">
                                            @csrf
                                            <input type="number" min="0" name="ammount"
                                                value="{{ $receipt_ammount }}" class="form-control d-inline w-50">
                                            <input type="text" name="receipt_id" value="{{ $receipt_id }}" hidden>
                                            <button role="submit" class="btn btn-warning"><i
                                                    class="fa-solid fa-pen-to-square"></i></button>
                                        </form>
                                    @else
                                        <a href="{{ route('employee.salary_create', $employee->id) }}"
                                            class="btn btn-success"><i class="fa-solid fa-money-bills"></i></a>
                                    @endif
                                </td>
                            @endif
                            <td>{{ $employee->created_at->format('Y-m-d') }}</td>
                            <td>
                                <a href="{{ route('employee.edit', $employee->id) }}" class="btn btn-info"><i
                                        class="fa-solid fa-user-pen"></i></a>
                                <a href="{{ route('employee.show', $employee->id) }}" class="btn btn-secondary"><i
